...
type s2Input struct {
	...
	Venue          string `json:"venue,omitempty" jsonschema:"description=按发表会议/期刊过滤,逗号分隔,如 NeurIPS,ICML,CVPR,ICLR,ACL;找顶会论文时填这个,由服务端精确过滤,别在 query 里凑会议名"`
...
func s2Search(ctx context.Context, apiKey string, in s2Input) (s2Output, error) {
	...
	// 要"可下载"时不走 S2 的 openAccessPdf 服务端过滤:S2 该字段覆盖率差(顶会论文常为空),
	// 一加就把大量本可经 arXiv 兜底下载的论文误筛成 0。改为客户端按兜底后的 pdf_url 是否非空筛,
	// 故多取候选(过滤会砍掉一部分)再截断到 n。
	limit := n
	...
	// year/venue/fields_of_study 直接透传给 S2 服务端精确筛,省得模型在 query 里凑关键词。
	if y := strings.TrimSpace(in.Year); y != "" {
		params.Set("year", y)
	}
	if v := strings.TrimSpace(in.Venue); v != "" {
		params.Set("venue", v)
	}
