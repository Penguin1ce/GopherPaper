...
export type TaskColumn = { status: string; label: string; count: number; items: TaskItem[] };
export type TaskBoard = { columns: TaskColumn[]; total: number };
export type TaskListResponse = { items: TaskItem[]; total: number; page: number; page_size: number };
...
export type TaskChecklistResponse = { items: TaskChecklistItem[]; total: number; done: number };
...
export type TaskDetail = {
  task: TaskItem;
  checklist: TaskChecklistResponse;
  comments: TaskComment[];
  activities: TaskActivityItem[];
};

export type TaskStatusCount = { status: string; count: number };
export type TaskPriorityCount = { priority: string; count: number };
export type TaskAssigneeCount = { assignee: string; count: number };
...
export type TaskFilter = { assignee?: string; priority?: string; query?: string };

function taskFilterQuery(filter: TaskFilter = {}) {
  const q = new URLSearchParams();
  if (filter.assignee?.trim()) q.set("assignee", filter.assignee.trim());
  if (filter.priority?.trim()) q.set("priority", filter.priority.trim());
  if (filter.query?.trim()) q.set("query", filter.query.trim());
  return q.toString();
}

export function fetchTaskBoard(token: string, filter: TaskFilter = {}) {
  const qs = taskFilterQuery(filter);
  return dashboardRequest<TaskBoard>(`/admin/tasks/board${qs ? `?${qs}` : ""}`, token);
}

export function fetchTaskList(
  token: string,
  params: TaskFilter & { status?: string; page?: number; page_size?: number } = {},
) {
  const q = new URLSearchParams();
  q.set("page", String(params.page ?? 1));
  q.set("page_size", String(params.page_size ?? 20));
  ...
  if (params.query?.trim()) q.set("query", params.query.trim());
  return dashboardRequest<TaskListResponse>(`/admin/tasks?${q}`, token);
}

export function fetchTaskStats(token: string) {
  return dashboardRequest<TaskStats>("/admin/tasks/stats", token);
}

export function createTask(token: string, body: TaskCreateBody) {
  return dashboardRequest<TaskItem>("/admin/tasks", token, { method: "POST", body: JSON.stringify(body) });
}

export function fetchTaskDetail(token: string, id: number) {
  return dashboardRequest<TaskDetail>(`/admin/tasks/${id}`, token);
}

export function updateTask(token: string, id: number, body: TaskUpdateBody) {
  return dashboardRequest<TaskItem>(`/admin/tasks/${id}`, token, { method: "PUT", body: JSON.stringify(body) });
}

export function moveTask(token: string, id: number, status: string, orderIdx?: number) {
  return dashboardRequest<TaskItem>(`/admin/tasks/${id}/move`, token, {
    method: "POST",
    body: JSON.stringify({ status, order_idx: orderIdx }),
  });
}

export function deleteTask(token: string, id: number) {
  return dashboardRequest<null>(`/admin/tasks/${id}`, token, { method: "DELETE" });
}

export type TaskBulkBody = {
  ids: number[];
...
export function bulkTasks(token: string, body: TaskBulkBody) {
  return dashboardRequest<BatchResult>("/admin/tasks/bulk", token, {
    method: "POST",
    body: JSON.stringify(body),
  });
}

export function fetchTaskComments(token: string, id: number) {
  return dashboardRequest<{ items: TaskComment[]; total: number }>(`/admin/tasks/${id}/comments`, token);
}
export function addTaskComment(token: string, id: number, content: string) {
  return dashboardRequest<TaskComment>(`/admin/tasks/${id}/comments`, token, {
    method: "POST",
    body: JSON.stringify({ content }),
  });
}
export function deleteTaskComment(token: string, id: number, commentId: number) {
  return dashboardRequest<null>(`/admin/tasks/${id}/comments/${commentId}`, token, { method: "DELETE" });
}

export function fetchChecklist(token: string, id: number) {
  return dashboardRequest<TaskChecklistResponse>(`/admin/tasks/${id}/checklist`, token);
}
export function addChecklistItem(token: string, id: number, content: string) {
  return dashboardRequest<TaskChecklistResponse>(`/admin/tasks/${id}/checklist`, token, {
    method: "POST",
    body: JSON.stringify({ content }),
  });
...
export function updateChecklistItem(
  ...
  return dashboardRequest<TaskChecklistResponse>(`/admin/tasks/${id}/checklist/${itemId}`, token, {
    method: "PUT",
    body: JSON.stringify(body),
  });
}
export function deleteChecklistItem(token: string, id: number, itemId: number) {
  return dashboardRequest<TaskChecklistResponse>(`/admin/tasks/${id}/checklist/${itemId}`, token, {
    method: "DELETE",
  });
}

export function fetchTaskActivities(token: string, id: number) {
  return dashboardRequest<{ items: TaskActivityItem[]; total: number }>(`/admin/tasks/${id}/activities`, token);
}

export function seedDemoTasks(token: string) {
  return dashboardRequest<{ created: number; message: string }>("/admin/tasks/demo/seed", token, {
    method: "POST",
  });
}
export function clearDemoTasks(token: string) {
  return dashboardRequest<{ created: number; message: string }>("/admin/tasks/demo/clear", token, {
    method: "POST",
  });
}

export async function downloadTasksCsv(token: string) {
