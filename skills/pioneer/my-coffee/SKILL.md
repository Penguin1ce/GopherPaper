...
4. **支付信息强约束**:优先使用 `payOrderQrCodeUrl`,用 Markdown 图片展示二维码并附完整可点击链接;返回中没有 `payOrderQrCodeUrl` 时,把 `payOrderUrl` 以 `[微信支付](weixin://开头的完整链接)` 链接形式输出,前端会自动渲染成可扫码的二维码卡片;禁止只给文字描述或让用户去 APP 查看支付码,禁止把 weixin 链接伪装成普通网页链接以外的形式输出。
...
6. **创建订单** - 调用 `createOrder`,必填 `deptId`、`productList`、`longitude`、`latitude`(用门店查询返回的坐标)。
   ...
   - 支付二维码用 Markdown 图片语法 `![支付二维码](完整payOrderQrCodeUrl)` 展示,并同步给出 `[打开支付二维码](完整payOrderQrCodeUrl)` 链接;没有 `payOrderQrCodeUrl` 时改输出 `[微信支付](完整payOrderUrl)` 链接,由前端渲染成扫码卡片。URL 必须原样完整保留,不得省略或截断。
