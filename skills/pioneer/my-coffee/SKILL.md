---
name: my-coffee
description: 当用户要点瑞幸咖啡、查瑞幸门店/商品、查取餐码/订单状态、取消瑞幸订单,或提到 瑞幸、luckin、咖啡、果茶、轻乳茶、点单、下单、门店、取餐码 时使用。依赖名为 my-coffee 的瑞幸订单 MCP 工具;下单、支付、取消等有后果操作必须按当前工具 schema 执行并先确认关键信息。
---
...
1. **Schema 优先**:以运行时 MCP 工具 schema 为单一真源,本文参数只是快速参考;工具名可能带 `my-coffee_` 前缀,按语义匹配同名能力。字段名、必填项或返回结构与本文不一致时,以 schema 和工具实际返回为准。
2. **下单顺序强约束**:`确认门店` -> `确认商品与下单意图` -> `previewOrder` ->(满足价格与明细校验)-> `createOrder`;不得跳步。确认必须来自用户明确回复,不要把含糊偏好或上一轮搜索意图当成下单确认。
...
7. **缺参追问约束**:门店/商品/订单未命中或参数不足时,只追问一个必要信息,避免并发追问。
...
10. **上下文复用约束**:只复用本会话最近且唯一的门店、商品、SKU、订单号。出现多个候选、用户换地点/商品、或时间间隔导致状态不可信时,重新确认。
...
1. 优先使用当前对话里最近且唯一的 `orderId`,没有或不确定则询问用户。
...
1. 优先使用当前对话里最近且唯一的 `orderId`,没有或不确定则询问用户。
2. 取消前先确认目标订单。能查到订单详情时,展示订单号、门店、商品、金额和状态;查不到详情时,至少展示订单号。
3. 询问用户是否确认取消。只有用户明确确认后才调用 `cancelOrder`;用户只是表达犹豫或询问能否取消时,不要调用。
4. 调用 `cancelOrder` 后简短说明取消结果;失败时说明可重试或去瑞幸 App 查看,不要反复自动重试。

## 工具参考

- 以下字段仅作快速参考;实际调用以运行时工具 schema 为准。
